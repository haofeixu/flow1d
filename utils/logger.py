...
class Logger:
    ...
    def add_init_flow_summary(self, init_flow, mode='train', tag='init_flow'):
        if self.total_steps % self.summary_freq == 0:
            init_flow = flow_tensor_to_image(init_flow[0])
            init_flow = torch.from_numpy(init_flow)

            self.summary_writer.add_image(mode + '/' + tag, init_flow, self.total_steps)
